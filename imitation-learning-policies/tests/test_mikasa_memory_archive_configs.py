...
def test_qframe_evidence_network_config_uses_new_bypass_target():
    cfg = _load_yaml(POLICY_DIR / "denoising_network" / "mikasa_evidence_memory_transformer.yaml")
    assert cfg["_target_"] == (
        "imitation_learning.models.denoising_networks."
        "mikasa_evidence_memory_transformer.MikasaEvidenceMemoryTransformer"
    )
    assert cfg["evidence_memory_enabled"] is True
    assert cfg["evidence_candidate_max_num"] == 8
    assert cfg["evidence_high_topk"] == 2
    assert cfg["evidence_low_topk"] == 2


def test_qframe_evidence_policy_config_preserves_mikasa_policy_and_uses_new_network():
    cfg = _load_yaml(POLICY_DIR / "diffusion_mikasa_qframe_evidence_memory_transformer.yaml")
    assert "diffusion_mikasa_memory_transformer" in cfg["defaults"]
    assert {
        "override denoising_network@denoising_network_partial": "mikasa_evidence_memory_transformer"
    } in cfg["defaults"]
    assert cfg["_target_"] == (
        "imitation_learning.policies.mikasa_history_denoising_policy."
        "MikasaHistoryDenoisingPolicy"
    ...
    network = cfg["denoising_network_partial"]
    assert network["evidence_candidate_max_num"] == 8


def test_qframe_evidence_intercept_config_raises_candidate_budget_to_twelve():
    cfg = _load_yaml(
        POLICY_DIR / "diffusion_mikasa_qframe_evidence_memory_transformer_intercept_medium.yaml"
    )
    assert "diffusion_mikasa_qframe_evidence_memory_transformer" in cfg["defaults"]
    network = cfg["denoising_network_partial"]
    assert network["evidence_candidate_max_num"] == 12


def test_qframe_longclip_config_adds_bypass_evidence_encoder():
    cfg = _load_yaml(POLICY_DIR / "diffusion_mikasa_qframe_longclip_evidence_memory_transformer.yaml")
    assert "diffusion_mikasa_qframe_evidence_memory_transformer" in cfg["defaults"]
    assert {"cond_encoder@history_evidence_feature_encoder": "multi_token_encoder"} in cfg["defaults"]
    encoder = cfg["history_evidence_feature_encoder"]["image_encoder_partial"]
    assert encoder["_target_"] == (
        "imitation_learning.models.encoders.longclip_image_encoder."
        "LongCLIPImageEncoder"
    )
    assert encoder["weights_path"].endswith("/Long-CLIP/longclip-L.pt")
